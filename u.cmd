...
REM Do NOT hide PS1 before launch (can break schtasks -File on some hosts)
attrib -h -s -r "%PS1%" >nul 2>&1

REM Single SYSTEM task: kill old PS, wait, then launch payload (no race with separate kill task)
...
echo Payload OK [%MARKER%], launched via single SYSTEM task (no kill race).
